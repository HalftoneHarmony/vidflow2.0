...
export function SupportPageClient({
    ...
    // Category icons and colors using translations
    const categoryConfig: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
        "general": { icon: HelpCircle, color: "text-blue-400 bg-blue-500/10 border-blue-500/30", label: t("filter_general") },
        "order": { icon: Package, color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30", label: t("filter_order") },
        "payment": { icon: CreditCard, color: "text-amber-400 bg-amber-500/10 border-amber-500/30", label: t("filter_payment") },
        "account": { icon: Users, color: "text-purple-400 bg-purple-500/10 border-purple-500/30", label: t("filter_account") },
        "service": { icon: Settings, color: "text-red-400 bg-red-500/10 border-red-500/30", label: t("filter_service") },
    };
    ...
    const handleFeedback = (faqId: number, type: 'like' | 'dislike') => {
        ...
        if (feedbackState[faqId] !== type) {
            toast.success(t("toast_feedback_success"));
        }
    };
    ...
    const validateField = (name: string, value: string): string | undefined => {
        switch (name) {
            case "name":
                ...
                if (value.length < 2) return t("validation_name_required") + " (2+ chars)"; // Rough fallback or add specific key
                break;
            case "email":
                if (!value.trim()) return t("validation_email_required");
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return t("validation_email_invalid");
                break;
            case "message":
                if (!value.trim()) return t("validation_message_required");
                if (value.length < 10) return t("validation_message_required") + " (10+ chars)";
                break;
        }
        return undefined;
    };
    ...
    const handleContactSubmit = async (e: React.FormEvent) => {
        ...
        if (errors.name || errors.email || errors.message) {
            toast.error(t("validation_error")); // Use generic error or specific "Form not valid"
            return;
        }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
                ...
                <div className="lg:col-span-1">
                    ...
                    <div className="flex lg:hidden gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4">
                        {[
                            { id: "faq", icon: HelpCircle, label: t("sidebar_faq") },
                            { id: "privacy", icon: Shield, label: t("sidebar_privacy") },
                            { id: "terms", icon: FileText, label: t("sidebar_terms") },
                            { id: "contact", icon: Mail, label: t("sidebar_contact") },
                        ].map((tab) => (
                            <button
                                key={tab.id}
                    ...
                    <div className="hidden lg:flex flex-col gap-2">
                        {[
                            { id: "faq", icon: HelpCircle, label: t("sidebar_faq"), desc: t("sidebar_faq") }, // desc duplicated?
                            { id: "privacy", icon: Shield, label: t("sidebar_privacy"), desc: t("sidebar_privacy_guide") },
                            { id: "terms", icon: FileText, label: t("sidebar_terms"), desc: t("sidebar_service_terms") },
                            { id: "contact", icon: Mail, label: t("sidebar_contact"), desc: t("sidebar_inquiry") },
                        ].map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id as Tab)}
                        ...
                        <Card className="mt-6 bg-gradient-to-br from-zinc-900/80 to-zinc-900/40 border-zinc-800 overflow-hidden relative">
                            ...
                            <CardContent className="p-5 space-y-4 relative">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-white text-sm">{t("contact_title")}</h3>
                                        <p className="text-xs text-zinc-500">{t("hours_title")}</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div>
                                    <CardTitle className="text-xl sm:text-2xl text-white font-[family-name:var(--font-oswald)] uppercase tracking-wide">
                                        {activeTab === "faq" && t("sidebar_faq")}
                                        {activeTab === "privacy" && privacyTitle}
                                        {activeTab === "terms" && termsTitle}
                                        {activeTab === "contact" && t("sidebar_contact")}
                                    </CardTitle>
                                    <CardDescription className="text-zinc-500 mt-1">
                                        {activeTab === "faq" && t("desc_faq")}
                                        {activeTab === "privacy" && t("desc_privacy")}
                                        {activeTab === "terms" && t("desc_terms")}
                                        {activeTab === "contact" && t("desc_contact")}
                                    </CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                                    ...
                                    <div className="space-y-3">
                                        {filteredFaqs.length > 0 ? (
                                                                    ...
                                                                    <div className="px-4 pb-4 pt-0">
                                                                        <div className="ml-11 pl-4 border-l-2 border-red-500/30 space-y-4">
                                                                            ...
                                                                            <div className="flex items-center gap-4 pt-2">
                                                                                <p className="text-xs text-zinc-500">{t("faq_feedback_question")}</p>
                                            ...
                                            <div className="text-center py-16 space-y-4">
                                                ...
                                                <div>
                                                    <p className="text-zinc-400 font-medium">{t("search_empty_title")}</p>
                                                    <p className="text-zinc-600 text-sm mt-1">{t("search_empty_desc")}</p>
                                                </div>
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                    ...
                                    {isSuccess && (
                                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in">
                                            <div className="bg-zinc-900 border border-emerald-500/50 rounded-xl p-8 text-center space-y-4 animate-in zoom-in-90">
                                                ...
                                                <div>
                                                    <h3 className="text-xl font-bold text-white">{t("validation_success")}</h3>
                                                    <p className="text-zinc-400 text-sm mt-1">{t("success_desc")}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
